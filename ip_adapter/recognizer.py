...
import time
from AnyText.ocr_recog.RecModel import RecModel
import torch
...
def load_ocr_model():
    rec_model_dir = "AnyText/ocr_weights/ppv3_rec.pth"
    predictor = create_predictor(rec_model_dir)
    args = edict()
    args.rec_image_shape = "3, 48, 320"
    args.rec_char_dict_path = 'AnyText/ocr_weights/ppocr_keys_v1.txt'
...
def main():
    ...
    # args.use_fp16 = False
    # text_recognizer = TextRecognizer(args, predictor)
    text_recognizer = load_ocr_model()
    text_recognizer.predictor.requires_grad_(False)
    image_dir = 'chun_data'
    os.makedirs('invalid_'+image_dir, exist_ok=True)
    image_file_list = get_image_file_list(image_dir)[:10]
    gt_text = ['春']*len(image_file_list)
    ...
    try:
        ...
        times = []
        for i in range(10):
            preds, pred_feats = text_recognizer.pred_imglist(img_list)  # get text
        ...
        for i in range(len(valid_image_file_list)):
            pred = preds_all[i]
            # if i > 1:
            #     print(preds_all[i] @ preds_all[i-1])
            order, idx = text_recognizer.decode(pred)
            text = text_recognizer.get_text(order)
            if loss[i] > 0.15:
                shutil.move(valid_image_file_list[i], 'invalid_'+valid_image_file_list[i])
                print("##!!move to ", 'invalid_'+valid_image_file_list[i])
